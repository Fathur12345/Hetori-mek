...
async function startAdrian() {
    ...
    const conn = connConnect({
        ...
        browser: ['HATORI BOT MD V1.2.0','Safari','1.2.0'],
   ...
   // anticall auto block
    ...
    conn.ev.on('messages.upsert', async chatUpdate => {
        ...
        require("./HatoriV1.2")(conn, m, chatUpdate, store)
        } catch (err) {
            console.log(err)
        }
    })
	...
	conn.ev.on("groups.update", async (json) => {
        ...
        try {
            ...
            for (let num of participants) {
               ...
               if (anu.action == 'add') {
                 let a =`*Welcome @${num.split("@")[0]}, Selamat Datang Di Group ${metadata.subject}*\n, Kamii Harap Kamu Adalah Member Permanent Kami, Dan Jangan Lupa Baca Desk Agar Tau Aturan...`
     ...
     text: a, 
      contextInfo: {
         ...
         renderLargerThumbnail: true
                    ...
                    let a = `*Dahh @${num.split("@")[0]}, Selamat Tinggal Dari Group ${metadata.subject}, Semoga Kamu Kembali Lagi Di Group Ini :)`
      conn.sendMessage(anu.id, {
     text: a, 
      contextInfo: {
         externalAdReply: {
         ...
         renderLargerThumbnail: true
                    ...
                    let a = `Gege @${num.split("@")[0]}, Sekarang Dah Jadi Admin Di Group ${metadata.subject} 🎉`
                    conn.sendMessage(anu.id, {
     text: a, 
      contextInfo: {
         externalAdReply: {
         ...
         renderLargerThumbnail: true
                    ...
                    let a = `Sabar Ae Bre Lu Di Demote Sama Admin Lain`
                    conn.sendMessage(anu.id, {
     text: a, 
      contextInfo: {
         externalAdReply: {
	...
	for (let i of kon) {
	    list.push({
	    	...
	    	vcard: `BEGIN:VCARD\nVERSION:3.0\nN:${namaowner}\nFN:${namaowner}\nitem1.TEL;waid=${i}:${i}\nitem1.X-ABLabel:Ponsel\nitem2.EMAIL;type=INTERNET:tessheroku24@gmail.com\nitem2.X-ABLabel:Email\nitem3.URL:https://youtube.com/@B16_OFC\nitem3.X-ABLabel:Channel YouTube\nitem4.ADR:;;Indonesia;;;;\nitem4.X-ABLabel:Region\nEND:VCARD`
	    })
	}
...
try{
		...
		if (update.connection == "connecting" || update.receivedPendingNotifications == "false") {
			lolcatjs.fromString(`[Sabar Lagi Connect]`)
		}
