...
module.exports = conn = async (conn, m, chatUpdate, store) => {
    try {
        ...
        const getCase = (cases) => {
            return "case  "+`'${cases}'`+fs.readFileSync("./HatoriV1.2.js").toString().split('case \''+cases+'\'')[1].split("break")[0]+"break"
        }
        const totalFitur = () =>{
            var mytext = fs.readFileSync("./HatoriV1.2.js").toString()
...
const result = {
status: 200,
author: `AdrianTzy Recode By B16_OFC`,
...
async function newReply(teks) {
        ...
        if (db.data.group[m.chat].antilink) {
            if (budy.match(`chat.whatsapp.com`)) {
                ...
                if (isgclink) return newReply(`Anda Beruntung Tidak Di Kick Oleh Bot Karna Lu Ngirim Link Grup Ini`)
                if (isAdmins) return newReply(`Eh Sorry Cuy Lupa Kalo Lu Admin 😁`)
                if (isCreator) return newReply(`Ga Jadi Lu Owner Gw Ternyata 😅`)
        ...
        if (db.data.group[m.chat].antilinkyt) {
            if (budy.match(`https://youtu.be`)) {
                ...
                if (!isBotAdmins) return newReply(`Ciaelah Bot Ga Admin`)
                if (isAdmins) return newReply(`Ehh Maaf Ternyata Kamu Admin 😁`)
                if (isCreator) return newReply(`Ehh Maaf Kamu Ownerku Ternyata 😅`)
                conn.groupParticipantsUpdate(m.chat, [m.sender], 'remove')
            }
...
}replygcXeoni(`Close Time ${q} Starting from now`)
setTimeout(() => {
var nomor = m.participant
const close = `*On time* Group Di Tutup Otomatis Oleh Bot/Admin\nSekarang Hanya Admin Yang Bisa Chat`
...
return replygcXeon('*Gunakan:*\nsecond\nminute\nhour\n\n*Example*\n10 second')
}
replygcXeon(`Auto Close ${q} Sudah Di Mulai Sekarang`)
setTimeout(() => {
var nomor = m.participant
...
break
        case 'cekapikey':
            if (!isCreator) return newReply(mess.owner)
            let lol = await fetchJson(`https://api.xyroinee.xyz/api/checkapikey?apikey=${apikey}`)
            newReply(mess.wait)
...
• *ᴇxᴘɪʀᴇᴅ:* ${lol.result.expired}`
        ...
        case 'enc': {
            if (!isCreator) return newReply(mess.owner)
            if (!q) return newReply(`Contoh ${prefix+command} const Hatori = require('Hatori-api')`)
        ...
        case 'react': {
                if (!isCreator) return newReply(mess.owner)
                if (!args[0]) return newReply(`Example: ${prefix + command} Kontol`)
                reactionMessage = {
                    react: {
                        text: args[0],
        ...
        case 'join': {
                if (!isCreator) return newReply(mess.owner)
                if (!text) return newReply('Mana Link Grup Nya?')
  ...
  conn.sendMessage(m.chat, {
    ...
    contextInfo: {
      externalAdReply: {
        ...
        body: 'B16_OFC',
        thumbnailUrl: "https://telegra.ph/file/b578ed5008b999d39ea80.jpg",
        sourceUrl: "https://chat.whatsapp.com/J3k6tngdHg4Dfxkx967axz",
        mediaType: 1,
...
break
            case 'sc':
            case 'script':
               newReply('Script Bot ${namabot} https://youtube.com/playlist?list=PLSvS3WbyjQEMni80sFW1sFcHnJV9PC3CY')
            break
        // Group Fitur
            case 'antilinkv2':
                if (!m.isGroup) return newReply(mess.group)
...
case 'broadcasttext': 
case 'broadcast': {
if (!ItsMeHatori) return replygc(mess.owner)
if (!q) return replygc(`Mana Teks Nya??*`)
...
( Q.S ${res.result.data.surah.name.transliteration.id} : ${res.result.data.number.inSurah} )`
            ...
            newReply(mess.wait)
			axios
				.get(`https://api.xyroinee.xyz/api/asmaulhusna?apikey=${apikey}`)
				.then(({ data }) => {
			...
			case 'alquranaudio': {
                ...
                newReply(mess.wait)
                conn.sendMessage(m.chat, { audio: { url: `https://api.xyroinee.xyz/api/quran/audio/${args[0]}?apikey=${apikey}`}, mimetype: 'audio/mp4', ptt: true }, { quoted: m })
            }
            break
			...
			newReply(mess.wait)
			axios
				.get(`https://api.xyroinee.xyz/api/quran/${args[0]}?apikey=${apikey}`)
				.then(({ data }) => {
            ...
            newReply(mess.wait)
            let anu = await fetchJson(`https://api.xyroinee.xyz/api/sholat/${text}?apikey=${apikey}`)
			...
			newReply(mess.wait)
			axios
				.get(`https://api.xyroinee.xyz/api/kisahnabi/${full_args}?apikey=${apikey}`)
				.then(({ data }) => {
            ...
            newReply(mess.wait)
			axios
				.get(`https://api.xyroinee.xyz/api/quran?apikey=${apikey}`)
				.then(({ data }) => {
					var text = 'List Surah:\n'
			...
			case 'pindownload': {
			    ...
			    let anu = axios.get(`https://api.xyroinee.xyz/api/pinterestdl?apikey=${apikey}&url=${full_args}`).then(({ data }) => {
			...
			newReply(mess.wait)
			axios
				.get(`https://api.xyroinee.xyz/api/ytsearch?apikey=${apikey}&query=${full_args}`)
				.then(({ data }) => {
					axios.get(`https://api.xyroinee.xyz/api/ytaudio2?apikey=${apikey}&url=https://www.youtube.com/watch?v=${data.result[0].videoId}`).then(({ data }) => {
			...
			newReply(mess.wait)
			axios
				.get(`https://api.xyroinee.xyz/api/ytaudio2?apikey=${apikey}&url=${args[0]}`)
				.then(({ data }) => {
					var caption = `❖ Title    : *${data.result.title}*\n`
					caption += `❖ Size     : *${data.result.size}*`
					conn.sendMessage(m.chat, { image: { url: data.result.thumbnail }, caption }).then(() => {
			...
			newReply(mess.wait)
			axios
				.get(`https://api.xyroinee.xyz/api/ytvideo2?apikey=${apikey}&url=${args[0]}`)
				.then(({ data }) => {
					var caption = `❖ Title    : *${data.result.title}*\n`
					caption += `❖ Size     : *${data.result.size}*`
					conn.sendMessage(m.chat, { image: { url: data.result.thumbnail }, caption }).then(() => {
			...
			newReply(mess.wait)
			axios.get(`https://api.xyroinee.xyz/api/tiktok?apikey=${apikey}&url=${args[0]}`).then(({ data }) => {
			...
			if (args.length == 0) return newReply(`Example: ${prefix + command} https://vt.tiktok.com/ZSwWCk5o/`)
			newReply(mess.wait)
			conn.sendMessage(m.chat, { audio: { url: `https://api.xyroinee.xyz/api/tiktokmusic?apikey=${apikey}&url=${args[0]}` }, mimetype: 'audio/mp4', fileName: `${data.result.title}.mp3` })
			...
			newReply(mess.wait)
			axios.get(`https://api.xyroinee.xyz/api/instagram?apikey=${apikey}&url=${args[0]}`).then(({ data }) => {
			...
			if (args.length == 0) return newReply(`Example: ${prefix + command} https://www.instagram.com/p/CJ8XKFmJ4al/?igshid=1acpcqo44kgkn`)
			newReply(mess.wait)
			axios.get(`https://api.xyroinee.xyz/api/instagram2?apikey=${apikey}&url=${args[0]}`).then(({ data }) => {
			...
			newReply(mess.wait)
			axios.get(`https://api.xyroinee.xyz/api/twitter?apikey=${apikey}&url=${args[0]}`).then(({ data }) => {
			...
			newReply(mess.wait)
			axios.get(`https://api.xyroinee.xyz/api/facebook?apikey=${apikey}&url=${args[0]}`).then(({ data }) => {
            ...
            db.data.users[m.sender].limit -= 3
            newReply(`🚩 3 Limit Used`)
			if (args.length == 0) return newReply(`Example: ${prefix + command} https://api.xyroinee.xyz`)
			conn.sendMessage(m.chat, { image: { url: `https://api.xyroinee.xyz/api/ssweb?apikey=${apikey}&url=${args[0]}`}, caption: `URL: ${args[0]}`}, { quoted: fkontak })
			break
            ...
            newReply(`🚩 3 Limit Used`)
			if (args.length == 0) return newReply(`Example: ${prefix + command} https://api.xyroinee.xyz`)
			conn.sendMessage(m.chat, { image: { url: `https://api.xyroinee.xyz/api/sswebfull?apikey=${apikey}&url=${args[0]}`}, caption: `URL: ${args[0]}`}, { quoted: m})
			break
            ...
            newReply(`🚩 3 Limit Used`)
			if (args.length == 0) return newReply(`Example: ${prefix + command} https://api.xyroinee.xyz`)
			axios.get(`https://api.xyroinee.xyz/api/ouoshortlink?apikey=${apikey}&url=${args[0]}`).then(({ data }) => {
            ...
            conn.sendMessage(m.chat, { image: { url: `https://api.xyroinee.xyz/api/dall-e?apikey=${apikey}&text=${args[0]}` }, caption: `${args[0]}` }, {quoted:m})
            }
            break
case 'ai': case 'clara': case 'openai':
try {
if (!text) return replygc(`Contoh:\n${prefix}${command} Apa Itu WhatsApp`)
...
break
			...
			newReply(mess.wait)
			var { data } = await axios.get(`https://api.xyroinee.xyz/api/kbbi?apikey=${apikey}&query=${full_args}`)
		...
		case 'brainly':{
			...
			newReply(mess.wait)
			var { data } = await axios.get(`https://api.xyroinee.xyz/api/brainly?apikey=${apikey}&query=${full_args}`)
			...
			if (args.length == 0) return newReply(`Example: ${prefix + command} siapakah sukarno`)
			newReply(mess.wait)
			var { data } = await axios.get(`https://api.xyroinee.xyz/api/roboguru?apikey=${apikey}&query=${full_args}&grade=sma&subject=sejarah`).catch((err) => console.error(err?.response?.data))
			...
			newReply(mess.wait)
			var { data } = await axios.get(`https://api.xyroinee.xyz/api/jaraktempuh?apikey=${apikey}&kota1=${titt1}&kota2=${titt2}`)
			...
			newReply(mess.wait)
			var { data } = await axios.get(`https://api.xyroinee.xyz/api/wiki?apikey=${apikey}&query=${full_args}`)
			newReply(data.result)
			break
			...
			args.shift()
			var tittt = args.join(' ')
			var { data } = await axios.get(`https://api.xyroinee.xyz/api/translate/auto/${kode_negara}?apikey=${apikey}&text=${tittt}`)
			...
			var { data } = await axios.get(`https://api.xyroinee.xyz/api/jadwaltv/${args[0]}?apikey=${apikey}`)
			newReply(mess.wait)
            ...
            db.data.users[m.sender].limit -= 5
            newReply(`🚩 5 Limit Used`)
			var { data } = await axios.get(`https://api.xyroinee.xyz/api/jadwaltv/now?apikey=${apikey}`)
			newReply(mess.wait)
            ...
            db.data.users[m.sender].limit -= 5
            newReply(`🚩 5 Limit Used`)
			var { data } = await axios.get(`https://api.xyroinee.xyz/api/newsinfo?apikey=${apikey}`)
			newReply(mess.wait)
			var titttttt = 'Result :\n'
			for (var x of data.result) {
...
break
            ...
            db.data.users[m.sender].limit -= 5
            newReply(`🚩 5 Limit Used`)
			var { data } = await axios.get(`https://api.xyroinee.xyz/api/cnnindonesia?apikey=${apikey}`)
			newReply(mess.wait)
            ...
            db.data.users[m.sender].limit -= 5
            newReply(`🚩 5 Limit Used`)
			var { data } = await axios.get(`https://api.xyroinee.xyz/api/cnnindonesia/nasional?apikey=${apikey}`)
			newReply(mess.wait)
            ...
            db.data.users[m.sender].limit -= 5
            newReply(`🚩 5 Limit Used`)
			var { data } = await axios.get(`https://api.xyroinee.xyz/api/cnnindonesia/internasional?apikey=${apikey}`)
			newReply(mess.wait)
            ...
            db.data.users[m.sender].limit -= 5
            newReply(`🚩 5 Limit Used`)
			var { data } = await axios.get(`https://api.xyroinee.xyz/api/infogempa?apikey=${apikey}`)
			newReply(mess.wait)
			...
			newReply(mess.wait)
			var { data } = await axios.get(`https://api.xyroinee.xyz/api/lirik?apikey=${apikey}&query=${full_args}`)
			newReply(data.result)
			break
			...
			newReply(mess.wait)
			var { data } = await axios.get(`https://api.xyroinee.xyz/api/cuaca/${args[0]}?apikey=${apikey}`)
			...
			newReply(mess.wait)
			var { data } = await axios.get(`https://api.xyroinee.xyz/api/kodepos?apikey=${apikey}&query=${full_args}`)
			...
			var { data } = await axios.get(`https://api.xyroinee.xyz/api/jadwalbola?apikey=${apikey}`)
			newReply(mess.wait)
			...
			newReply(mess.wait)
			var { data } = await axios.get(`https://api.xyroinee.xyz/api/genshin/${full_args}?apikey=${apikey}`)
	        ...
	        let smeme = `https://api.xyroinee.xyz/api/memecreator?apikey=${apikey}&text1=${bawah}&text2=${atas}&img=${fatGans}`
            ...
            case 'attp':
                ...
                await conn.sendMessage(m.chat, {sticker: {url:`https://api.xyroinee.xyz/api/attp?apikey=${apikey}&text=${full_args}` }}, { quoted: m })
            } catch (e) {
                newReply(mess.error)
            }
            break
            ...
            newReply(`🚩 5 Limit Used`)
                try {
                    if (args.length == 0) return newReply(`Example: ${prefix + command} AdrianTzy`)
                    await conn.sendMessage(m.chat, {sticker: {url:`https://api.xyroinee.xyz/api/attp2?apikey=${apikey}&text=${full_args}` }}, { quoted: m })
                } catch (e) {
                    newReply(mess.error)
            }
            break
            ...
            newReply(`🚩 5 Limit Used`)
                try {
                    if (args.length == 0) return newReply(`Example: ${prefix + command} AdrianTzy`)
                    await conn.sendMessage(m.chat, {sticker: {url:`https://api.xyroinee.xyz/api/ttp?apikey=${apikey}&text=${full_args}` }}, { quoted: m })
                } catch (e) {
                    newReply(mess.error)
            }
            break
			...
			newReply(mess.wait)
			axios.get(`https://api.xyroinee.xyz/api/stalkig/${args[0]}?apikey=${apikey}`).then(({ data }) => {
			...
			newReply(mess.wait)
			axios.get(`https://api.xyroinee.xyz/api/stalktiktok/${args[0]}?apikey=${apikey}`).then(({ data }) => {
				var caption = `Username : ${data.result.username}\n`
			...
			newReply(mess.wait)
			var { data } = await axios.get(`https://api.xyroinee.xyz/api/mobilelegend/${args[0]}?apikey=${apikey}`)
			newReply(data.result)
			...
			if (args.length == 0) return newReply(`Example: ${prefix + command} AdrianTzy`)
			newReply(mess.wait)
			axios.get(`https://api.xyroinee.xyz/api/github/${args[0]}?apikey=${apikey}`).then(({ data }) => {
			...
			newReply(mess.wait)
			axios.get(`https://api.xyroinee.xyz/api/twitter/${args[0]}?apikey=${apikey}`).then(({ data }) => {
            ...
            newReply(`🚩 5 Limit Used`)
			newReply(mess.wait)
                conn.sendMessage(m.chat, { image: { url: `https://api.xyroinee.xyz/api/meme/darkjoke?apikey=${apikey}`}, caption: `Done?`}, {quoted: m})
            break
            ...
            newReply(`🚩 5 Limit Used`)
			newReply(mess.wait)
                conn.sendMessage(m.chat, { image: { url: `https://api.xyroinee.xyz/api/random/meme?apikey=${apikey}`}, caption: `Done?`}, {quoted: m})
            break
            ...
            newReply(`🚩 5 Limit Used`)
                newReply(mess.wait)
                conn.sendMessage(m.chat, { image: { url: `https://api.xyroinee.xyz/api/meme/memeindo?apikey=${apikey}`}, caption: `Done?`}, {quoted: m})
            break
			...
			case 'bucincert':
				...
				newReply(mess.wait)
				kueri = args.join(" ")
                conn.sendMessage(m.chat, { image: { url: `https://api.xyroinee.xyz/api/bucinserti?apikey=${apikey}&name=${kueri}`}, caption: 'Sertifikatnya kack'}, {quoted: m})
            break
			...
			newReply(mess.wait)
			ytta = args.join(" ")
            conn.sendMessage(m.chat, { image: { url: `https://api.xyroinee.xyz/api/toloserti?apikey=${apikey}&name=${ytta}`}, caption: 'Sertifikatnya kack'}, {quoted: m})
            break
            ...
            conn.sendMessage(m.chat, { image: { url: `https://api.xyroinee.xyz/api/pacarserti?apikey=${apikey}&name1=${nik}&name2=${prov}`}, caption: titidnya}, {quoted: m})
            break
	        ...
	        case 'carbon':
	            ...
	            newReply(mess.wait)
	            conn.sendMessage(m.chat, { image: { url: `https://api.xyroinee.xyz/api/carbon?apikey=${apikey}&code=${q}&language=nodejs`}, caption: `Created By AdrianTzy\n\n\nCode:\n\n${q}`}, {quoted: m})
	        break
		...
		case 'freefire':
			...
			if (args.length == 0) return newReply(`Example: ${prefix + command} AdrianTzy `)
			newReply(mess.wait)
			conn.sendMessage(m.chat, { image: { url: `https://api.xyroinee.xyz/api/ephoto1/${command}?apikey=${apikey}&text=${args}`}, caption: `Created By AdrianTzy\n\n Type: ${command}\n\nText: ${args}`}, { quoted: m })
			break
		...
		case 'carvedwood':
			...
			if (args.length == 0) return newReply(`Example: ${prefix + command} AdrianTzy`)
			newReply(mess.wait)
			conn.sendMessage(m.chat, { image: { url: `https://api.xyroinee.xyz/api/photooxy1/${command}?apikey=${apikey}&text=${args}`}, caption: `Created By AdrianTzy\n\n Type: ${command}\n\nText: ${args}`}, { quoted: m})
			break
    ...
    // Text Prome
		...
		case 'wallgravity':
			...
			if (args.length == 0) return newReply(`Example: ${prefix + command} AdrianTzy`)
			newReply(mess.wait)
			conn.sendMessage(m.chat, { image: { url: `https://api.xyroinee.xyz/api/textprome2/${command}?apikey=${apikey}&text1=${args[0]}&text2=${args[1]}`}, caption: `Created By AdrianTzy\n\n Type: ${command}\n\nText: ${args}`}, { quoted: m })
			break
		...
		case 'thunder':
			...
			if (args.length == 0) return newReply(`Example: ${prefix + command} AdrianTzy`)
			newReply(mess.wait)
			conn.sendMessage(m.chat, { image: { url: `https://api.xyroinee.xyz/api/textprome/${command}?apikey=${apikey}&text=${args}` }, caption: `Created By AdrianTzy\n\n Type: ${command}\n\nText: ${args}`}, { quoted: m })
			break
        ...
        break
			...
			newReply(mess.wait)
                if (isMedia) {
                    ...
                    const anu = await TelegraPh(media)
                    await 
                    conn.sendMessage(m.chat, { image: { url: `https://api.xyroinee.xyz/api/imagetoanime?apikey=${apikey}&img=${anu}` }, caption: mess.done }, { quoted: m })
                } else {
                newReply('Reply Gambarnya Tod..')
                }
            }
            break
			...
			case 'nomerhoki': case 'nomorhoki': {
                ...
                db.data.users[m.sender].limit -= 5
                newReply(`🚩 5 Limit Used`)
                if (!Number(text)) return newReply(`Contoh : ${prefix + command} 6281260431003`)
...
return replaygcXeon('*Gunakan:*\nseccond\nminute\nhour\n\n*Contoh*\n.closetime 5 seccond')
}
replygcxeon(`Close Time ${q} Sudah Di Mulai Sekarang`)
setTimeout(() => {
var nomor = m.participant
const close = `*Sudah Waktunya..* Group Sudah Di Tutup Oleh Admin\nSekarang Hanya Admin Yang Dapat Mengirim Pesan!`
XeonBotInc.groupSettingUpdate(from, 'announcement')
replygcxeon(close)
}, timer)
}
...
└───────𖦹Ꜥꜥ`}
   ${readmore}
┌──𖦹Ꜥꜥ 𝐍 𝐄 𝐖  𝐅 𝐄 𝐀 𝐓 𝐔 𝐑 𝐄
│${prefix}Ai
│${prefix}closetime
│${prefix}opentime
└───────𖦹Ꜥꜥ

┌──𖦹Ꜥꜥ *O W N E R  M E N U*
│ ${prefix}enc *<text>*
...
┌──𖦹Ꜥꜥ *G R O U P  M E N U*
│ ${prefix}closetime *<example>*
│ ${prefix}opentime *<example>*
│ ${prefix}kick *<@user>*
│ ${prefix}add *<@user>*
│ ${prefix}culik *<@user>*
│ ${prefix}promote *<@user>*
...
└───────𖦹Ꜥꜥ`
        ...
        console.log("Eror Di Bagian HatoriV1.2.js "+util.format(err))
    }
}
